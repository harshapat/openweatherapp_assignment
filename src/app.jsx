...
var forecast_list = []; //Forecast List

//////////////////////////////////////////////////////
var ForecastListRender = React.createClass({
render: function() {
  ...
  return (
    <div className="weather-grid-wrapper">
      {this.props.list.map(function(listValue) {
        var dt = new Date(listValue.dt * 1000)

        return <div className="weather-grid-wrapper-box">
                <div className=""><span className="weather-heading">{dt.toGMTString()}</span></div>
                <div className="temp"><span className="temp-number">Temperature: {Math.round(listValue.main.temp - 273.15)} C</span><span className="wi wi-degrees"></span></div>
                <div className="humidity"><i className="wi wi-raindrop"></i>Humidity: {listValue.main.humidity} %</div>
                <div className="wind"><i className="wi wi-small-craft-advisory"></i>Wind Speed: {listValue.wind.speed} <span className="vel">Km/h</span></div>
            </div>
      })}
    </div>
  )
...
var Weather = React.createClass({
    ...
    fetchData: function() {
        ...
        if (citiesWeather[currentCity]) {
            ...
            .then(function(data) {
                ...
                this.updateData();
                console.log("forecast_list: ", forecast_list);
            }.bind(this));
        }
    },
    ...
    updateData: function() {
        ...
        this.setState({
            weather: citiesWeather[currentCity].city.id,
            temp: Math.round(citiesWeather[currentCity].list[0].main.temp - 273.15), // Kelvin to Celcius
    ...
    render: function() {
        ...
        return <div className={bgColorClass}>
            ...
            <div className="weather">
             ...
             <ForecastListRender list={forecast_list} />  
        </div>
    }
});
